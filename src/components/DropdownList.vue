<template>
    <div>
        <div class="dropdown">
            <b-form-input v-if="Object.keys(selectedItem).length === 0" ref="dropdowninput" v-model.trim="inputValue"
                          class="dropdown-input custom-select" type="text" placeholder="Type to search"/>
            ...
            <div v-show="inputValue && apiLoaded" class="dropdown-list">
                ...
                <b-dropdown-item @click="selectItem(item)" v-show="itemVisible(item)" v-for="item in itemList"
                                 :key="item.id"
                >
        ...
        <b-dropdown boundary="window" text="Select stuff" class="" style="max-height: 200px; overflow-x: hidden;">
        ...
        <datalist id="my-list-id">
            <option v-for="(item, index) in itemList" v-bind:key="index">{{ item.game.name }}</option>
        </datalist>
    </div>
</template>
